...
public class MapBolt implements IRichBolt {
    ...
    public void prepare(Map<String,String> stormConf, 
        ...
        String string = "noOfWorkers";
        String string1 = "mapExecutors";
        String string2 = "spoutExecutors";
        int counter = Integer.parseInt(stormConf.get(string));
        neededVotesToComplete = (counter - 1) * (Integer.parseInt(stormConf.get(string1))) + Integer.parseInt(stormConf.get(string2));
    }
    ...
    public synchronized void execute(Tuple input) {
        ...
        if(neededVotesToComplete == 0){
            for(String k : myContext.content.keySet()){
                for(String v : myContext.content.get(k)){
                    this.collector.emit(new Values<Object>(k, v));
                }
            }
